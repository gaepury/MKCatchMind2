...
public class WaitingRoom extends JFrame implements ActionListener{
	...
	public void actionPerformed(ActionEvent e){
		...
		if(ob == text){
			...
			String msg = CatchMindProtocol.WAITING_ROOM_CHAT+";" + id + ";" + text.getText();
			text.setText("");
			try {
				pw.println(msg);
				pw.flush();
		...
		// 방만들기 버튼을 만든다면 이제 게임방(그림그리는 방 출력해줘야됨)
			...
			String msg = CatchMindProtocol.MAKE_GAME_ROOM+";" + id + ";" + roomName + ";";
			...
			gameRoom.init();
			gameRoom.setResizable(false); // ### 방크기 조절 못하게 함.
			...
			String msg = CatchMindProtocol.EXIT_WATTING_ROM+";" + id + ";";
			try {
				pw.println(msg);
				pw.flush();
				System.exit(1);
			...
			String msg = CatchMindProtocol.JOIN_GAME_ROOM+";" + id + ";" + roomName + ";";
			try {
				pw.println(msg);
				pw.flush();
			} catch (Exception e2) {
